...
public class ResourceIntTest {
	...
	public void testHelloAndGoodbyeWithBothInjections() {
		DefaultGatewayRequest requestJavax = createRequest("/api/hello-goodbye-javax");
		...
		DefaultGatewayRequest requestGuice = createRequest("/api/hello-goodbye-guice");
		GatewayResponse responseGuice0 = handler.handleRequest(requestGuice, context);
		GatewayResponse responseGuice1 = handler.handleRequest(requestGuice, context);

		assertEquals("Hello, #0\nGoodbye, #0", responseJavax0.getBody());
		assertEquals("Hello, #1\nGoodbye, #0", responseJavax1.getBody());
		assertEquals("Hello, #2\nGoodbye, #0", responseGuice0.getBody());
		assertEquals("Hello, #3\nGoodbye, #0", responseGuice1.getBody());
	}
